...
call init_coarser_mesh_statP(nprobe,nprobe,nprobe,.true.)
call parameter()
        ...
        z=(real(k)-0.5)/real(nzm)
         do j=ph1%zst(2),ph1%zen(2)
            ...
            do i=ph1%zst(1),ph1%zen(1)
             ...
             pp3(i,j,k) =(cos(1.*pi*x)+cos(2.*pi*x)+cos(3.*pi*x)+cos(4.*pi*x)+&
                  cos(5.*pi*x)+cos(6.*pi*x)+cos(7.*pi*x))
             po3(i,j,k)=pp3(i,j,k)
           end do
         end do
      end do
         ...
         do j=ph1%zst(2),ph1%zen(2)
            do i=ph1%zst(1),ph1%zen(1)
               tmp1=tmp1+(pp3(i,j,k)-po3(i,j,k))**2
                !if (nrank==0) write(*,*) i,j,k, pp3(i,j,k),po3(i,j,k), pp3(i,j,k)-po3(i,j,k)
            enddo
         enddo
      enddo
...
!!   call PROBE(ux1,uy1,uz1,phi1,uprobe,uprobe_write,vprobe_write,wprobe_write,phiprobe_write)
