...
!  real(mytype), private, parameter :: PI = 3.14159265358979323846_mytype
  ...
  ! wave numbers
  complex(mytype), save, allocatable, dimension(:,:,:) :: kxyz,kxyzY
  ...
  complex(mytype), allocatable, dimension(:,:,:) :: cw1,cw1b,cw2,cw2b,cw2c
...
contains
  ...
  subroutine poisson(rhs, bcx, bcy, bcz)
    ...
    if (bcx==0 .and. bcy==0 .and. bcz==0) then
       if (istret.ne.0) call matrice_refinement()
...
if (nclx==0) then
   do i=1,nx/2+1
      ...
      xkx(i)=cmplx(nx*wp/xlx,nx*wp/xlx)
      exs(i)=cmplx(nx*w/xlx,nx*w/xlx)
      xk2(i)=cmplx((nx*wp/xlx)**2,(nx*wp/xlx)**2)
   enddo
   do i=nx/2+2,nx
      xkx(i)=xkx(nx-i+2)
      exs(i)=exs(nx-i+2)
   ...
   do i=1,nx
      ...
      wp=acix6*2.*dx*sin(w/2.)+(bcix6*2.*dx)*sin(3./2.*w)
      wp=wp/(1.+2.*alcaix6*cos(w))
      xkx(i)=cmplx(nxm*wp/xlx,nxm*wp/xlx)
      exs(i)=cmplx(nxm*w/xlx,nxm*w/xlx)
      xk2(i)=cmplx((nxm*wp/xlx)**2,(nxm*wp/xlx)**2)
   enddo
   xkx(1)=0.
   exs(1)=0.
   xk2(1)=0.
...
if (ncly==0) then
   do j=1,ny/2+1
      ...
      if (istret==0) yky(j)=cmplx(ny*wp/yly,ny*wp/yly)
      if (istret.ne.0) yky(j)=cmplx(ny*wp,ny*wp)
      eys(j)=cmplx(ny*w/yly,ny*w/yly)
      yk2(j)=cmplx((ny*wp/yly)**2,(ny*wp/yly)**2)
   enddo
   do j=ny/2+2,ny
      yky(j)=yky(ny-j+2)
      eys(j)=eys(ny-j+2)
   ...
   do j=1,ny
      ...
      wp=aciy6*2.*dy*sin(w/2.)+(bciy6*2.*dy)*sin(3./2.*w)
      wp=wp/(1.+2.*alcaiy6*cos(w))
      if (istret==0) yky(j)=cmplx(nym*wp/yly,nym*wp/yly)
      if (istret.ne.0) yky(j)=cmplx(nym*wp,nym*wp)
      eys(j)=cmplx(nym*w/yly,nym*w1/yly)
      yk2(j)=cmplx((nym*wp/yly)**2,(nym*wp/yly)**2)
   enddo
   yky(1)=0.
   eys(1)=0.
   yk2(1)=0.
...
if (nclz==0) then
   do k=1,nz/2+1
      ...
      zkz(k)=cmplx(nz*wp/zlz,nz*wp/zlz)
      ezs(k)=cmplx(nz*w/zlz,nz*w/zlz)
      zk2(k)=cmplx((nz*wp/zlz)**2,(nz*wp/zlz)**2)
   enddo
else
   do k=1,nz/2+1
      w=2.*pi*0.5*(k-1)/nzm
      ...
      zkz(k)=cmplx(nzm*wp/zlz,-nzm*w1p/zlz)
      ezs(k)=cmplx(nzm*w/zlz,nzm*w1/zlz)
      zk2(k)=cmplx((nzm*wp/zlz)**2,(nzm*w1p/zlz)**2)
   enddo
endif
!
!if (nrank==0) then
...
do i = sp%yst(1), sp%yen(1)
   xtt=cmplx((bicix6*2.*cos(real(exs(i))*3.*dx/2.)+&
        cicix6*2.*cos(real(exs(i))*5.*dx/2.)),&
        (bicix6*2.*cos(real(exs(i))*3.*dx/2.)+&
        cicix6*2.*cos(real(exs(i))*5.*dx/2.)))
   ytt=cmplx((biciy6*2.*cos(real(eys(j))*3.*dy/2.)+&
        ciciy6*2.*cos(real(eys(j))*5.*dy/2.)),&
        (biciy6*2.*cos(real(eys(j))*3.*dy/2.)+&
        ciciy6*2.*cos(real(eys(j))*5.*dy/2.)))
   ztt=cmplx((biciz6*2.*cos(real(ezs(k))*3.*dz/2.)+&
        ciciz6*2.*cos(real(ezs(k))*5.*dz/2.)),&
        (biciz6*2.*cos(real(ezs(k))*3.*dz/2.)+&
        ciciz6*2.*cos(real(ezs(k))*5.*dz/2.)))
   xtt1=cmplx((aicix6*2.*cos(real(exs(i))*dx/2.)),&
        (aicix6*2.*cos(real(exs(i))*dx/2.)))
   ytt1=cmplx((aiciy6*2.*cos(real(eys(j))*dy/2.)),&
        (aiciy6*2.*cos(real(eys(j))*dy/2.)))
   ztt1=cmplx((aiciz6*2.*cos(real(ezs(k))*dz/2.)),&
        (aiciz6*2.*cos(real(ezs(k))*dz/2.)))
   xt1=cmplx((1.+2.*ailcaix6*cos(real(exs(i))*dx)),&
        (1.+2.*ailcaix6*cos(real(exs(i))*dx)))
   yt1=cmplx((1.+2.*ailcaiy6*cos(real(eys(j))*dy)),&
        (1.+2.*ailcaiy6*cos(real(eys(j))*dy)))
   zt1=cmplx((1.+2.*ailcaiz6*cos(real(ezs(k))*dz)),&
        (1.+2.*ailcaiz6*cos(real(ezs(k))*dz)))
...
!   print *,i,j,k, kxyz(i,j,k)
      ...
      do i = sp%xst(1),sp%xen(1)  
         ...
         yt1=cmplx((1.+2.*ailcaiy6*cos(real(eys(j))*dy)),&
              (1.+2.*ailcaiy6*cos(real(eys(j))*dy)))
         zt1=cmplx((1.+2.*ailcaiz6*cos(real(ezs(k))*dz)),&
              (1.+2.*ailcaiz6*cos(aimag(ezs(k))*dz)))
         
         tmp1=cmplx(real(ztt1+ztt)/real(zt1),aimag(ztt1+ztt)/aimag(zt1))
         tmp2=cmplx(real(ytt1+ytt)/real(yt1),real(ytt1+ytt)/real(yt1))
         tmp3=cmplx(real(xtt1+xtt)/real(xt1),real(xtt1+xtt)/real(xt1))
         
         tmp4=cmplx((real(tmp1)*real(tmp2))**2,(aimag(tmp1)*aimag(tmp2))**2)
         tmp5=cmplx((real(tmp1)*real(tmp3))**2,(aimag(tmp1)*aimag(tmp3))**2)
         tmp6=cmplx((real(tmp3)*real(tmp2))**2,(aimag(tmp3)*aimag(tmp2))**2)
         
         tmp1=cmplx(real(tmp4)*real(xk2(i)),aimag(tmp4)*aimag(xk2(i)))
         tmp2=cmplx(real(tmp5)*real(yk2(j)),aimag(tmp5)*aimag(yk2(j)))
         tmp3=cmplx(real(tmp6)*real(zk2(k)),aimag(tmp6)*aimag(zk2(k)))
...
do i = sp%yst(1),sp%yen(1)
   ...
   xt1=cmplx((1.+2.*ailcaix6*cos(real(exs(i))*dx)),&
        (1.+2.*ailcaix6*cos(real(exs(i))*dx)))
   transx(i)=cmplx(real(xtt1+xtt)/real(xt1),real(xtt1+xtt)/real(xt1))!(xtt+xtt)/xt1
enddo
do j = 1,ny!sp%yst(2),sp%yen(2)
   ytt=cmplx((biciy6*2.*cos(real(eys(j))*3.*dy/2.)+&
        ciciy6*2.*cos(real(eys(j))*5.*dy/2.)),&
        (biciy6*2.*cos(real(eys(j))*3.*dy/2.)+&
        ciciy6*2.*cos(real(eys(j))*5.*dy/2.)))
   ...
   yt1=cmplx((1.+2.*ailcaiy6*cos(real(eys(j))*dy)),&
        (1.+2.*ailcaiy6*cos(real(eys(j))*dy)))
   transy(j)=cmplx(real(ytt1+ytt)/real(yt1),real(ytt1+ytt)/real(yt1))!(ytt+ytt)/yt1
enddo
do k = sp%yst(3),sp%yen(3)
   ztt=cmplx((biciz6*2.*cos(real(ezs(k))*3.*dz/2.)+&
        ciciz6*2.*cos(real(ezs(k))*5.*dz/2.)),&
   ...
   zt1=cmplx((1.+2.*ailcaiz6*cos(real(ezs(k))*dz)),&
        (1.+2.*ailcaiz6*cos(real(ezs(k))*dz)))
   transz(k)=cmplx(real(ztt1+ztt)/real(zt1),aimag(ztt1+ztt)/aimag(zt1))!(ztt+ztt)/zt1
enddo
...
!
   ...
   do j=1,ny/2
   do i=sp%yst(1),sp%yen(1)
      cw1(i,j,k)=cmplx(real(yky(2*j-1))*real(transx(i))*real(transz(k)),&
           aimag(yky(2*j-1))*aimag(transx(i))*aimag(transz(k)))
      cw2(i,j,k)=cmplx(real(yky(2*j))*real(transx(i))*real(transz(k)),&
           aimag(yky(2*j))*aimag(transx(i))*aimag(transz(k)))
   enddo
   enddo
   enddo
...
!main diagonal 
   ...
   do j=2,ny/2-1
   do i=sp%yst(1),sp%yen(1)
      a(i,j,k,3)=cmplx(-(real(xk2(i))*real(transy(2*j-1))*real(transy(2*j-1))*real(transz(k))*real(transz(k)))&
           -(real(zk2(k))*real(transy(2*j-1))*real(transy(2*j-1))*real(transx(i))*real(transx(i)))&
           -real(cw1(i,j,k))*real(cw1(i,j,k))*xa0*xa0-&
           xa1*xa1*(real(cw1(i,j,k))*real(cw1(i,j-1,k))+real(cw1(i,j,k))*real(cw1(i,j+1,k))),&
           ...
           -aimag(cw1(i,j,k))*aimag(cw1(i,j,k))*xa0*xa0-&
           xa1*xa1*(aimag(cw1(i,j,k))*aimag(cw1(i,j-1,k))+aimag(cw1(i,j,k))*aimag(cw1(i,j+1,k))))
!      print *,i,j,k,a(i,j,k,3),kxyz(i,j,k)
      a2(i,j,k,3)=cmplx(-(real(xk2(i))*real(transy(2*j))*real(transy(2*j))*real(transz(k))*real(transz(k)))&
           -(real(zk2(k))*real(transy(2*j))*real(transy(2*j))*real(transx(i))*real(transx(i)))&
           -real(cw2(i,j,k))*real(cw2(i,j,k))*xa0*xa0-&
           xa1*xa1*(real(cw2(i,j,k))*real(cw2(i,j-1,k))+real(cw2(i,j,k))*real(cw2(i,j+1,k))),&
           ...
           xa1*xa1*(aimag(cw2(i,j,k))*aimag(cw2(i,j-1,k))+aimag(cw2(i,j,k))*aimag(cw2(i,j+1,k))))
   enddo
   enddo
   do i=sp%yst(1),sp%yen(1)
      a(i,1,k,3)=cmplx(-(real(xk2(i))*real(transy(1))*real(transy(1))*real(transz(k))*real(transz(k)))&
           -(real(zk2(k))*real(transy(1))*real(transy(1))*real(transx(i))*real(transx(i)))&
           -real(cw1(i,1,k))*real(cw1(i,1,k))*xa0*xa0-xa1*xa1*(real(cw1(i,1,k))*real(cw1(i,2,k))),&
           ...
           -aimag(cw1(i,1,k))*aimag(cw1(i,1,k))*xa0*xa0-xa1*xa1*(aimag(cw1(i,1,k))*aimag(cw1(i,2,k))))
      a(i,ny/2,k,3)=cmplx(-(real(xk2(i))*real(transy(ny-2))*real(transy(ny-2))*real(transz(k))*real(transz(k)))&
           -(real(zk2(k))*real(transy(ny-2))*real(transy(ny-2))*real(transx(i))*real(transx(i)))&
           -real(cw1(i,ny/2,k))*real(cw1(i,ny/2,k))*xa0*xa0-&
           xa1*xa1*(real(cw1(i,ny/2,k))*real(cw1(i,ny/2-1,k))),&
           ...
           -aimag(cw1(i,ny/2,k))*aimag(cw1(i,ny/2,k))*xa0*xa0-&
           xa1*xa1*(aimag(cw1(i,ny/2,k))*aimag(cw1(i,ny/2-1,k))))
      a2(i,1,k,3)=cmplx(-(real(xk2(i))*real(transy(2))*real(transy(2))*real(transz(k))*real(transz(k)))&
           -(real(zk2(k))*real(transy(2))*real(transy(2))*real(transx(i))*real(transx(i)))&
           -real(cw2(i,1,k))*real(cw2(i,1,k))*(xa0-xa1)*(xa0+xa1)-xa1*xa1*(real(cw2(i,1,k))*real(cw2(i,2,k))),&
           ...
           -aimag(cw2(i,1,k))*aimag(cw2(i,1,k))*(xa0-xa1)*(xa0+xa1)-xa1*xa1*(aimag(cw2(i,1,k))*aimag(cw2(i,2,k))))
      a2(i,ny/2,k,3)=cmplx(-(real(xk2(i))*real(transy(ny-1))*real(transy(ny-1))*real(transz(k))*real(transz(k)))&
           -(real(zk2(k))*real(transy(ny-1))*real(transy(ny-1))*real(transx(i))*real(transx(i)))&
           -real(cw2(i,ny/2,k))*real(cw2(i,ny/2,k))*(xa0+xa1)*(xa0+xa1)-&
           xa1*xa1*(real(cw2(i,ny/2,k))*real(cw2(i,ny/2-1,k))),&
           ...
           xa1*xa1*(aimag(cw2(i,ny/2,k))*aimag(cw2(i,ny/2-1,k))))
   enddo
   enddo
...
!sup diag +1
   ...
   do j=2,ny/2-1
   do i=sp%yst(1),sp%yen(1)   
      a(i,j,k,4)=cmplx(xa0*xa1*(real(cw1(i,j,k))*real(cw1(i,j+1,k))+real(cw1(i,j+1,k))*real(cw1(i,j+1,k))),&
           xa0*xa1*(aimag(cw1(i,j,k))*aimag(cw1(i,j+1,k))+aimag(cw1(i,j+1,k))*aimag(cw1(i,j+1,k))))
      a2(i,j,k,4)=cmplx(xa0*xa1*(real(cw2(i,j,k))*real(cw2(i,j+1,k))+real(cw2(i,j+1,k))*real(cw2(i,j+1,k))),&
           xa0*xa1*(aimag(cw2(i,j,k))*aimag(cw2(i,j+1,k))+aimag(cw2(i,j+1,k))*aimag(cw2(i,j+1,k))))
   enddo
   enddo
   do i=sp%yst(1),sp%yen(1)   
      a(i,1,k,4)=2.*cmplx((xa0*xa1*(real(cw1(i,1,k))*real(cw1(i,2,k))+real(cw1(i,2,k))*real(cw1(i,2,k)))),&
           (xa0*xa1*(aimag(cw1(i,1,k))*aimag(cw1(i,2,k))+aimag(cw1(i,2,k))*aimag(cw1(i,2,k)))))
      a2(i,1,k,4)=cmplx((xa0-xa1)*xa1*(real(cw2(i,1,k))*real(cw2(i,2,k)))&
           +xa0*xa1*(real(cw2(i,2,k))*real(cw2(i,2,k))),&
           (xa0-xa1)*xa1*(aimag(cw2(i,1,k))*aimag(cw2(i,2,k)))&
           +xa0*xa1*(aimag(cw2(i,2,k))*aimag(cw2(i,2,k))))
      a2(i,ny/2-1,k,4)=cmplx(xa0*xa1*(real(cw2(i,ny/2-1,k))*real(cw2(i,ny/2,k)))&
           +(xa0+xa1)*xa1*(real(cw2(i,ny/2,k))*real(cw2(i,ny/2,k))),&
           xa0*xa1*(aimag(cw2(i,ny/2-1,k))*aimag(cw2(i,ny/2,k)))&
           +(xa0+xa1)*xa1*(aimag(cw2(i,ny/2,k))*aimag(cw2(i,ny/2,k))))
      a2(i,ny/2,k,4)=0.
   enddo
   enddo
...
!sup diag +2
   ...
   do i=sp%yst(1),sp%yen(1)   
   do j=1,ny/2-2
      a(i,j,k,5)=cmplx(-real(cw1(i,j+1,k))*real(cw1(i,j+2,k))*xa1*xa1,&
           -aimag(cw1(i,j+1,k))*aimag(cw1(i,j+2,k))*xa1*xa1)
      a2(i,j,k,5)=cmplx(-real(cw2(i,j+1,k))*real(cw2(i,j+2,k))*xa1*xa1,&
           -aimag(cw2(i,j+1,k))*aimag(cw2(i,j+2,k))*xa1*xa1)
   enddo
   a(i,1,k,5)=cmplx(real(a(i,1,k,5))*2.,aimag(a(i,1,k,5))*2.)
   a(i,ny/2-1,k,5)=0.
   a(i,ny/2,k,5)=0.
   a2(i,ny/2-1,k,5)=0.
   a2(i,ny/2,k,5)=0. 
...
!inf diag -1
   ...
   do i=sp%yst(1),sp%yen(1)   
   do j=2,ny/2
      a(i,j,k,2)=cmplx(xa0*xa1*(real(cw1(i,j,k))*real(cw1(i,j-1,k))+real(cw1(i,j-1,k))*real(cw1(i,j-1,k))),&
           xa0*xa1*(aimag(cw1(i,j,k))*aimag(cw1(i,j-1,k))+aimag(cw1(i,j-1,k))*aimag(cw1(i,j-1,k))))
      a2(i,j,k,2)=cmplx(xa0*xa1*(real(cw2(i,j,k))*real(cw2(i,j-1,k))+real(cw2(i,j-1,k))*real(cw2(i,j-1,k))),&
           xa0*xa1*(aimag(cw2(i,j,k))*aimag(cw2(i,j-1,k))+aimag(cw2(i,j-1,k))*aimag(cw2(i,j-1,k))))
   enddo
   a(i,1,k,2)=0.
   a2(i,1,k,2)=0.
   a2(i,2,k,2)=cmplx(xa0*xa1*(real(cw2(i,2,k))*real(cw2(i,1,k)))&
        +(xa0+xa1)*xa1*(real(cw2(i,1,k))*real(cw2(i,1,k))),&
        xa0*xa1*(aimag(cw2(i,2,k))*aimag(cw2(i,1,k)))&
        +(xa0+xa1)*xa1*(aimag(cw2(i,1,k))*aimag(cw2(i,1,k))))
   a2(i,ny/2,k,2)=cmplx((xa0+xa1)*xa1*(real(cw2(i,ny/2,k))*real(cw2(i,ny/2-1,k)))&
        +xa0*xa1*(real(cw2(i,ny/2-1,k))*real(cw2(i,ny/2-1,k))),&
        (xa0+xa1)*xa1*(aimag(cw2(i,ny/2,k))*aimag(cw2(i,ny/2-1,k)))&
        +xa0*xa1*(aimag(cw2(i,ny/2-1,k))*aimag(cw2(i,ny/2-1,k))))
   enddo
   enddo
!inf diag -2
   do k=sp%yst(3),sp%yen(3)
   do i=sp%yst(1),sp%yen(1)  
   do j=3,ny/2
      a(i,j,k,1)=cmplx(-real(cw1(i,j-1,k))*real(cw1(i,j-2,k))*xa1*xa1,&
           -aimag(cw1(i,j-1,k))*aimag(cw1(i,j-2,k))*xa1*xa1)
      a2(i,j,k,1)=cmplx(-real(cw2(i,j-1,k))*real(cw2(i,j-2,k))*xa1*xa1,&
           -aimag(cw2(i,j-1,k))*aimag(cw2(i,j-2,k))*xa1*xa1)
   enddo
   a(i,1,k,1)=0.
   a(i,2,k,1)=0.
...
!   do k=sp%yst(3),sp%yen(3)
!   do i=sp%yst(1),sp%yen(1)
!      if ((real(xkx(i))==0.).and.(real(zkz(k)==0))) then
         a(1,1,1,3)=cmplx(1.,1.)
         a(1,1,1,4)=0.
         a(1,1,1,5)=0.
!      endif
...
!   
   ...
   do j=1,ny
   do i=sp%yst(1),sp%yen(1)
      cw1(i,j,k)=cmplx(real(yky(j))*real(transx(i))*real(transz(k)),&
           aimag(yky(j))*aimag(transx(i))*aimag(transz(k)))
   enddo
   enddo
   enddo
...
!main diagonal 
   ...
   do j=2,ny-1
   do i=sp%yst(1),sp%yen(1)
      a3(i,j,k,3)=cmplx(-(real(xk2(i))*real(transy(j))*real(transy(j))*real(transz(k))*real(transz(k)))&
           -(real(zk2(k))*real(transy(j))*real(transy(j))*real(transx(i))*real(transx(i)))&
           -real(cw1(i,j,k))*real(cw1(i,j,k))*xa0*xa0-&
           xa1*xa1*(real(cw1(i,j,k))*real(cw1(i,j-1,k))+real(cw1(i,j,k))*real(cw1(i,j+1,k))),&
   ...
   do k=sp%yst(3),sp%yen(3)
   do i=sp%yst(1),sp%yen(1)
      a3(i,1,k,3)=cmplx(-(real(xk2(i))*real(transy(1))*real(transy(1))*real(transz(k))*real(transz(k)))&
           -(real(zk2(k))*real(transy(1))*real(transy(1))*real(transx(i))*real(transx(i)))&
           -real(cw1(i,1,k))*real(cw1(i,1,k))*xa0*xa0-xa1*xa1*(real(cw1(i,1,k))*real(cw1(i,2,k))),&
           -(aimag(xk2(i))*aimag(transy(1))*aimag(transy(1))*aimag(transz(k))*aimag(transz(k)))&
           -(aimag(zk2(k))*aimag(transy(1))*aimag(transy(1))*aimag(transx(i))*aimag(transx(i)))&
           -aimag(cw1(i,1,k))*aimag(cw1(i,1,k))*xa0*xa0-xa1*xa1*(aimag(cw1(i,1,k))*aimag(cw1(i,2,k))))
      a3(i,ny,k,3)=cmplx(-(real(xk2(i))*real(transy(ny))*real(transy(ny))*real(transz(k))*real(transz(k)))&
           -(real(zk2(k))*real(transy(ny))*real(transy(ny))*real(transx(i))*real(transx(i)))&
           -real(cw1(i,ny,k))*real(cw1(i,ny,k))*xa0*xa0-&
           xa1*xa1*(real(cw1(i,ny,k))*real(cw1(i,ny-1,k))),&
           ...
           -aimag(cw1(i,ny,k))*aimag(cw1(i,ny,k))*xa0*xa0-&
           xa1*xa1*(aimag(cw1(i,ny,k))*aimag(cw1(i,ny-1,k))))
   enddo
   enddo
!sup diag +1
   do k=sp%yst(3),sp%yen(3)
   do i=sp%yst(1),sp%yen(1)
   do j=2,ny-1
      a3(i,j,k,4)=cmplx(xa0*xa1*(real(cw1(i,j,k))*real(cw1(i,j+1,k))+real(cw1(i,j+1,k))*real(cw1(i,j+1,k))),&
           xa0*xa1*(aimag(cw1(i,j,k))*aimag(cw1(i,j+1,k))+aimag(cw1(i,j+1,k))*aimag(cw1(i,j+1,k))))
   enddo
   a3(i,1,k,4)=cmplx((xa0*xa1*(real(cw1(i,1,k))*real(cw1(i,2,k))+real(cw1(i,2,k))*real(cw1(i,2,k)))),&
        (xa0*xa1*(aimag(cw1(i,1,k))*aimag(cw1(i,2,k))+aimag(cw1(i,2,k))*aimag(cw1(i,2,k)))))
   enddo
   enddo
!sup diag +2
   ...
   do i=sp%yst(1),sp%yen(1)
   do j=1,ny-2
      a3(i,j,k,5)=cmplx(-real(cw1(i,j+1,k))*real(cw1(i,j+2,k))*xa1*xa1,&
           -aimag(cw1(i,j+1,k))*aimag(cw1(i,j+2,k))*xa1*xa1)
   enddo
...
!inf diag -1
   ...
   do i=sp%yst(1),sp%yen(1)
   do j=2,ny
      a3(i,j,k,2)=cmplx(xa0*xa1*(real(cw1(i,j,k))*real(cw1(i,j-1,k))+real(cw1(i,j-1,k))*real(cw1(i,j-1,k))),&
           xa0*xa1*(aimag(cw1(i,j,k))*aimag(cw1(i,j-1,k))+aimag(cw1(i,j-1,k))*aimag(cw1(i,j-1,k))))
   enddo
   a3(i,1,k,2)=0.
   enddo
...
!inf diag -2
   ...
   do i=sp%yst(1),sp%yen(1)
   do j=3,ny
      a3(i,j,k,1)=cmplx(-real(cw1(i,j-1,k))*real(cw1(i,j-2,k))*xa1*xa1,&
           -aimag(cw1(i,j-1,k))*aimag(cw1(i,j-2,k))*xa1*xa1)
   enddo
   a3(i,1,k,1)=0.
   a3(i,2,k,1)=0.
