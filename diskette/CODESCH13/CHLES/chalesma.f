c *********************************************************************
c  This code is derived by the code in CODESCH9/CHA for the DNS of
c  turbulent plane channel.
c  This version performs simulations of LES with three different kinds
c  of subgrid models.
c  Smagorinsky
c  Smagorinsky with eddy viscosity constant on planes
c  Dynamic Model with Lilley contraction
c  No-model this is an important test to perform
c  to understand the effects of the numerical viscosity
c**************************************************************
      program main
      ...
      read(15,*) ifield,inicos
      read(15,*) csma,iles
      read(15,*) ichy2,str2o
      cvisc=1./ren
c
c     ifield=0 Reads the inner field  
c     ifield=1 Reads the velocity distribution on the lower wall
c
      pi=2.*asin(1.)
      n1m=n1-1
      n2m=n2-1
...
c     the equations for the Cartesian component u(i) i=1,3
...
c
      ...
      call indic
c  
c   evaluates the distribution of mesh points 
c   in the normal direction and the metrics
c
      call coordi(y)
      call metric(y)
c
...
c
c  distribution of localised transpiration on the lower wall
c
      call slotin
      call inirea(ntii,time,q,ru,pr)
      if(icfl.eq.1) then
c
c   courant number calculation for the restarting field 
c
      call cfl(q,cflmm)
      dt=cflc/cflmm
      cflm=cflc
...
c
      call enerca(q,ene0,pr)
c
c  check the divergence
c
      call divgck(q,qmax)
      write(6,7698) qmax,ntime
      if(qmax.gt..1e-03) go to 169
       call outh(ntime,time,cflm,ene0)
      endif
c
...
c
      ...
      enddo
c
c   initiation for the "pressure solver"
c
      call phini(qcap,dph)
c
      ntstf=ntii+ntst
...
c
      if(iles.eq.0) then
       print *,'calcolo con modello dinamico inibito'
       do 320 k=1,n3m
       do 320 j=1,n2m
       do 320 i=1,n1m
       vis(i,j,k)=cvisc
 320  continue
      ...
      if(iles.eq.1) then
      print *,'iles',iles
       print * ,'calcolo di LES modello dinamico'
       print * ,'filtro fisico '
                     end if
      if(iles.eq.3) then
      print *,'iles',iles
       print * ,'calcolo di LES modello di Smagorinsky'
                    end if
      if(iles.eq.5) then
      print *,'iles',iles
...
c
  ...
  368 format(3x,'end calcul for dt=',e12.4)
      ...
      call tschem(q,pr,ru,qcap,dph,time,ncount) 
c
      time=time+dt
