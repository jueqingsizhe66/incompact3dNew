...
c  this is related to the FFT to solve the streamfunction in the
c  initial conditions for stability vortex structures
c
      subroutine fftqup
      include 'param.f'
      parameter (m1m=m1-1,m1mh=m1m/2+1)
...
c
      subroutine dsolps(n2m,qk,n1mh)
      include 'param.f'
      parameter (m1m=m1-1,m1mh=m1m/2+1)
      dimension qi(m1mh,m2),si(m1mh,m2),qei(m1mh,m2)
      dimension qk(m1mh,m2)
      call tripps(qk,qi,si,qei,1,n2m,1,n1mh)
      return
      end
c
c   ********************* subr pscalc
...
c  solver in x2
c
      subroutine pscalc(qcap,psi)
      include 'param.f'
      parameter (m1m=m1-1,m1mh=m1m/2+1,m1md=m1m+2)
      common/dim/n1,n1m,n2,n2m,n3,n3m
      dimension qkr(m1mh,m2),qki(m1mh,m2)
     1         ,qcap(m1,m2),psi(m1,m2)
      common/fftcop/ifxz(13),trigz(3*m1m/2+1)
      real xr(m1+1,m2),work(m1,m2)
      n1mh=n1m/2+1
      n1md=n1m+2
      n1mdu=n1md+1
      n1mu=n1m-1
      ...
      xr(1,j)=-qcap(n1m,j)
c
c  fft-1 applied to the vor(i,j)=qcap(i,j)
...
c
      call dsolps(n2m,qkr,n1mh)
c
c   solution of poisson equation immag. part
c
      call dsolps(n2m,qki,n1mh)
c
...
c  ****************************** subrout tripps **********************
c
      subroutine tripps( f,q,s,qe,j1,j2,k1,k2)
      include 'param.f'
c
      parameter (m1m=m1-1,m1mh=m1m/2+1)
...
c
   ...
   11 continue
      do 24 k=k1,k2
      fnum=(fn(k) - c(j2)*qe(k,j1) - a(j2)*qe(k,j2-1))
      fden=(c(j2)*s(k,j1) + a(j2)*s(k,j2-1)  +b(k,j2))
      write(96,*)k,fnum,fden
