...
c    of the computational ones x1,x2,x3
c    In this code the transformation is trivial
c
      subroutine coordi
c
      include 'param.f'
      ...
      common/corsta/y1s(m1),y2s(m2),y3s(m3)
      common/mesh/dx1,dx1q,dx2,dx2q,dx3,dx3q
      common/ispec/imic
      pi=2.*asin(1.)
c
...
c
   ...
   63 continue
      do i=1,n1m
      y1s(i)=(yp1(i)+yp1(i+1))*0.5
      enddo
      do j=1,n2m
      y2s(j)=(yp2(j)+yp2(j+1))*0.5
      enddo
      open(57,file='2dcoord.dat',form='unformatted')
      write(57)n1m,n2m,1
      write(57) ((y1s(i),i=1,n1m),j=1,n2m)
...
c  ****************************** subrout divgck  **********************
c  this subroutine perform the calculation of the maximum of divg(q)
c
...
c  boundaries with periodic conditions.
c
      subroutine indic
c
      include 'param.f'
...
c   THIS should be used when a statistical steady state is
c   considered.
c   For isotropic decaying turbulence can not be called
c
      subroutine oldqua
c
      include 'param.f'
...
c   HOLDS what asserted in the previous routine
c
      subroutine taver
c
      include 'param.f'
...
c  in order to check whether the calculation proceeds regularly
c
      do 311 l=1,ndv
      vmaxo(l)=vmax(l)
      vmax(l)=0.
