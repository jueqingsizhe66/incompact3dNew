...
c  along x3 and x1 to use the real fourier transform in x1
c  tridiagonal periodic solver in x2
c
   ...
   12 continue
c
c   real  fft applied to the divg(qhat) by fft99
...
c
  ...
  33  continue
c
c   now cfft99 for the x3 direction
c
...
c
   ...
   21 continue
c
c   fft applied to the phi by cfft99
...
c
      if(n1m.gt.1) then 
c
c    now fft99 is applied to get real field in ohysical space
c
      call fft99(xr,work,trigx1,ifx1,1,m1+1,n1m,n3m,+1)
                   endif
      do 22 i=1,n1m
